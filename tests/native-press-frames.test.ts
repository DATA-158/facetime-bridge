...
beforeAll(async () => {
  ...
  executable = join(temporary, "press-and-frames-tests");
  const compile = Bun.spawnSync(
    [
      ...
      join(root, "native-tests", "PressAndFramesTests.swift"),
      "-framework",
      "AppKit",
      "-framework",
      "ApplicationServices",
...
describe("native press/frames fixtures (issue #6)", () => {
  test("compiles the press/frames fixture", () => {
    expect(compileExitCode, compileStderr).toBe(0);
  });

  test("press matching, newest-match law, and frames emitter hold", () => {
    expect(compileExitCode, compileStderr).toBe(0);
    const result = Bun.spawnSync([executable], { stdout: "pipe", stderr: "pipe" });
    expect(result.exitCode, result.stderr.toString()).toBe(0);
    expect(result.stdout.toString()).toContain("native press/frames regressions passed");
  });
});